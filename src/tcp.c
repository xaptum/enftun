...
static int
enftun_tcp_native_connect(struct enftun_tcp* tcp,
                          const char* host,
                          const char* port,
                          int fwmark)
{
...
void
enftun_tcp_native_init(struct enftun_tcp* tcp)
{
    tcp->ops  = enftun_tcp_native_ops;
    tcp->type = ENFTUN_TCP_NATIVE;
}
